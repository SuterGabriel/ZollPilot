...
test('CLS-01 gleiche HS-6 auf allen Belegen, auch mit längeren Codes', () => {
  ...
  assert.equal(pruefeCLS01(akte, R, DEFAULTS).status, 'ok');
});
...
test('CLS-01 Abweichung auf der Packliste', () => {
  ...
  const b = pruefeCLS01(akte, R, DEFAULTS);
...
test('CLS-01 Abweichung im Präferenznachweis', () => {
  ...
  assert.match(pruefeCLS01(akte, R, DEFAULTS).begruendung, /Präferenznachweis 848420/);
});

test('CLS-01 fehlender HS-Code auf der Rechnung ist ein Verstoß, fehlende Packliste nicht', () => {
  assert.equal(pruefeCLS01(akteAus({ rechnung: { positionen: [{ nr: 1 }] } }), R, DEFAULTS).status, 'verletzt');
  assert.equal(pruefeCLS01(akteAus({ rechnung: { positionen: [{ nr: 1, hs6: '841330' }] } }), R, DEFAULTS).status, 'ok');
  assert.equal(pruefeCLS01(akteAus({ packliste: { positionen: [] } }), R, DEFAULTS).status, 'nicht_pruefbar');
});
