...
test('QTY-02 Toleranz ist das Größere aus 2 kg und 1 %', () => {
  assert.equal(pruefeQTY02(mit(100, 102), R).status, 'ok', '2 kg absolut bei kleiner Sendung');
  assert.equal(pruefeQTY02(mit(100, 102.5), R).status, 'verletzt');
  assert.equal(pruefeQTY02(mit(1000, 1010), R).status, 'ok', '1 % bei großer Sendung');
  assert.equal(pruefeQTY02(mit(1000, 1010.5), R).status, 'verletzt');
});

test('QTY-02 bleibt weich unterhalb der Eskalationsschwelle, wird hart darüber', () => {
  const weich = pruefeQTY02(mit(1000, 1050), R);
  assert.equal(weich.status, 'verletzt');
  assert.equal(weich.haerte_effektiv, 'soft');
  const hart = pruefeQTY02(mit(1000, 1100), R);
...
test('QTY-02 Toleranz kommt aus dem Katalog', () => {
  ...
  assert.equal(pruefeQTY02(mit(100, 100.1), streng).status, 'verletzt');
});

test('QTY-02 fehlendes B/L-Gewicht nicht prüfbar', () => {
  assert.equal(pruefeQTY02(akteAus({ packliste: { brutto_gesamt_kg: 100 } }), R).status, 'nicht_pruefbar');
});
